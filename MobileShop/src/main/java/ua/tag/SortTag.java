...
public class SortTag extends SimpleTagSupport {
	...
	public boolean isParamValuePresent(Map<String, String[]> map) {
		return map.entrySet().stream().filter(entry -> entry.getKey().equals(SORT)).map(Map.Entry::getValue).flatMap((array) -> Arrays.stream(array)).anyMatch((str) -> str.equals(paramValue));
	}
